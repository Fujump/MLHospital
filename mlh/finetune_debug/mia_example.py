...
if __name__ == "__main__":
    ...
    if attack_type == "label-only":
        ...
        # Split target_train_loader dataset
        dataset = target_train_loader.dataset
        dataset_len = len(dataset)
        subset1_len = int(dataset_len * args.split_ratio)
        subset2_len = dataset_len - subset1_len
        subset1, subset2 = random_split(dataset, [subset1_len, subset2_len])
        train_loader_finetune = DataLoader(subset1, batch_size=128, shuffle=True, num_workers=2)
        train_loader_attack = DataLoader(subset2, batch_size=128, shuffle=True, num_workers=2)

        # Split target_test_loader dataset
        dataset = target_test_loader.dataset
        dataset_len = len(dataset)
        subset1_len = int(dataset_len * args.split_ratio)
        subset2_len = dataset_len - subset1_len
        subset1, subset2 = random_split(dataset, [subset1_len, subset2_len])
        test_loader_finetune = DataLoader(subset1, batch_size=128, shuffle=True, num_workers=2)
        test_loader_attack = DataLoader(subset2, batch_size=128, shuffle=True, num_workers=2)

        # Split shadow_train_loader dataset
        dataset = shadow_train_loader.dataset
        dataset_len = len(dataset)
        subset1_len = int(dataset_len * args.split_ratio)
        subset2_len = dataset_len - subset1_len
        subset1, subset2 = random_split(dataset, [subset1_len, subset2_len])
        shadow_train_loader_finetune = DataLoader(subset1, batch_size=128, shuffle=True, num_workers=2)
        shadow_train_loader_attack = DataLoader(subset2, batch_size=128, shuffle=True, num_workers=2)

        # Split shadow_test_loader dataset
        dataset = shadow_test_loader.dataset
        dataset_len = len(dataset)
        subset1_len = int(dataset_len * args.split_ratio)
        subset2_len = dataset_len - subset1_len
        subset1, subset2 = random_split(dataset, [subset1_len, subset2_len])
        shadow_test_loader_finetune = DataLoader(subset1, batch_size=128, shuffle=True, num_workers=2)
        shadow_test_loader_attack = DataLoader(subset2, batch_size=128, shuffle=True, num_workers=2)

        # Update loaders for attack
        target_train_loader = train_loader_attack
        target_test_loader = test_loader_attack
        shadow_train_loader = shadow_train_loader_attack
        shadow_test_loader = shadow_test_loader_attack
        ...
        if attack_type == "augmentation":
            ...
            attack_dataset_rotation_finetune = AugemtaionAttackDataset( args, "rotation" , target_model, shadow_model,
                                            train_loader_finetune.dataset, test_loader_finetune.dataset, shadow_train_loader_finetune.dataset, shadow_test_loader_finetune.dataset,args.device)
            
            attack_dataset_translation_finetune =AugemtaionAttackDataset( args, "translation" , target_model, shadow_model,
                                            train_loader_finetune.dataset, test_loader_finetune.dataset, shadow_train_loader_finetune.dataset, shadow_test_loader_finetune.dataset,args.device)
            ...
            attack_dataset_finetune = AttackDataset(args, attack_type, target_model, shadow_model,
                                        train_loader_finetune, test_loader_finetune, shadow_train_loader_finetune, shadow_test_loader_finetune)


        # train attack model
        print("auditing remain set...")
        ...
        print("auditing finetune set...")
        if "black-box" in attack_type:
            attack_model = BlackBoxMIA(
                num_class=args.num_class,
                device=args.device,
                attack_type=attack_type,
                attack_train_dataset=attack_dataset_finetune.attack_train_dataset,
                attack_test_dataset=attack_dataset_finetune.attack_test_dataset,
                batch_size=128)
        elif ("metric-based" in attack_type) or ("white-box" in attack_type):
            attack_model = MetricBasedMIA(
                num_class=args.num_class,
            ...
            attack_model = DataAugmentationMIA(
                num_class = attack_dataset_rotation_finetune.attack_train_dataset.data.shape[1],
                device = args.device, 
                attack_type= "rotation",
                attack_train_dataset=attack_dataset_rotation_finetune.attack_train_dataset,  
                attack_test_dataset= attack_dataset_rotation_finetune.attack_train_dataset,  
                # save_path= save_path, 
                batch_size= 128)
            attack_model = DataAugmentationMIA(
                num_class = attack_dataset_translation_finetune.attack_train_dataset.data.shape[1],
                device = args.device, 
                attack_type= "translation",
                attack_train_dataset=attack_dataset_translation_finetune.attack_train_dataset,  
                attack_test_dataset= attack_dataset_translation_finetune.attack_test_dataset,
                # save_path= save_path, 
                batch_size= 128)
